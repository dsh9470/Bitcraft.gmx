// base terrain generation script
// based on realtutsGML

/*
Modifications:
-not dependent on starting room height
-global sea level (supposedly); independent rock-layer generation
*/

// variables

// TESTING: changing to be room_height-independent - changed room_height refs to variable
...
for(xx = start_room; xx < end_room; xx += 32)
{
    ...
    treespawn = floor(random(10));
    if(treespawn == 0 && alth < global.sealevel)
        with(instance_create(xx, alth-32, obj_tree))
        {
            canGrow = true;
        }    

    instance_create(xx, alth, obj_grass);
  ...
  //vary terrain
  //todo: smooth transition between chunks with global vars?
